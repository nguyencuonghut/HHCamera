    ...
    <section class="content">
        <div class="container-fluid">
            <div class="row">
                <div class="col-12">
                    <div class="card">
                    <form class="form-horizontal" method="post" action="{{ route('admin.devices.update', $device->id) }}" name="edit_device" id="edit_device" novalidate="novalidate">
                        @method('PATCH')
                        {{ csrf_field() }}
                        <!-- /.card-header -->
                        <div class="card-body">
                                        ...
                                        <div class="controls">
                                            <select name="farm_id" id="farm_id" class="form-control select2">
                                                ...
                                                @foreach($farms as $key => $value)
                                                    <option value="{{$key}}" {{ $key == $device->farm_id ? 'selected' : '' }}>{{$value}}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
